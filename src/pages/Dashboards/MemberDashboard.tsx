...
const MemberDashboard = () => {
  ...
  const [selectedShare, setSelectedShare] = useState<ShareWithCompany | null>(null);
  ...
  const { data: shares = [], isLoading: sharesLoading } = useQuery<ShareWithCompany[]>({
    queryKey: ["user_shares", user?.id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("user_shares")
        .select("*, companies(name, price_per_share, sector, logo_url)")
        .eq("user_id", user!.id)
        .order("purchase_date", { ascending: false });
      if (error) throw error;
      return (data || []) as ShareWithCompany[];
    },
    enabled: !!user,
    refetchInterval: 15000,
  });
  ...
  shares.forEach(sh => {
    const sec = sh.companies?.sector || "Autre";
    const price = Number(sh.companies?.price_per_share ?? sh.purchase_price);
    sectorMap[sec] = (sectorMap[sec] || 0) + price * sh.quantity;
  });
  ...
  const depositMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("wallet_transactions").insert({
        ...
        description: `Dépôt via ${paymentServices.find((s: any) => s.id === selectedService)?.name || "service"}`,
      });
      if (error) throw error;
    },
    onSuccess: async () => {
  ...
  const transferMutation = useMutation({
    mutationFn: () =>
      invokePlatformAction("transfer_money", {
        recipientMsnId: transferRecipient,
        amount: Number(amount),
  ...
  const p2pMutation = useMutation({
    mutationFn: () =>
      invokePlatformAction("create_listing", {
        userShareId: selectedShare?.id,
        quantity: Number(p2pQty),
        pricePerShare: Number(p2pPrice),
      }),
    onSuccess: async () => {
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="font-mono text-xs text-white/30 mt-0.5">
                {profile?.msn_id || "—"} · {liveTime.toLocaleTimeString("fr-FR")}
              </p>
            </div>
          ...
          <div className="flex gap-2 mt-5 flex-wrap">
            {[
              ...
              {
                label: "VENDRE", Icon: Send, color: "purple", fn: () => {
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={perfData}>
                    ...
                    <Tooltip
                      ...
                      contentStyle={{ background: "#0a0f1e", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 8, fontSize: 10, fontFamily: "monospace" }}
                ...
                <div className="rounded-2xl border border-white/5 p-5" style={{ background: "rgba(255,255,255,0.02)" }}>
                  ...
                  <ResponsiveContainer width="100%" height={160}>
                    <RechartsPie>
                      ...
                      <Tooltip
                        formatter={(v: any) => fmtCurrency(v)}
                        contentStyle={{ background: "#0a0f1e", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, fontSize: 10, fontFamily: "monospace" }}
                      />
                    </RechartsPie>
                  </ResponsiveContainer>
                    ...
                    <div key={tx.id} className="flex items-center justify-between p-4 hover:bg-white/2 transition-colors">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${tx.type === "deposit" ? "bg-emerald-500/10" : tx.type === "withdrawal" ? "bg-rose-500/10" : tx.type === "purchase" ? "bg-blue-500/10" : "bg-amber-500/10"}`}>
                          {tx.type === "deposit" ? <ArrowDownLeft className="h-3.5 w-3.5 text-emerald-400" /> :
                            tx.type === "withdrawal" ? <ArrowUpRight className="h-3.5 w-3.5 text-rose-400" /> :
                              tx.type === "purchase" ? <Coins className="h-3.5 w-3.5 text-blue-400" /> :
                      ...
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden">
                            {sh.companies?.logo_url
                              ? <img src={sh.companies.logo_url} alt="" className="w-full h-full object-cover" />
                              : <Building2 className="h-4 w-4 text-amber-400/50" />}
                          </div>
                          <div>
                            <p className="font-mono font-bold text-white/90 text-sm">{sh.companies?.name || "—"}</p>
                            <p className="font-mono text-[10px] text-white/30">{sh.companies?.sector || "—"} · {sh.order_number}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-6 flex-wrap">
                          ...
                          <button
                            onClick={() => {
                              setSelectedShare(sh);
                              setP2pQty(String(sh.quantity));
                              setP2pPrice(String(cp));
                              setP2pDialog(true);
                            }}
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${tx.type === "deposit" ? "bg-emerald-500/10" : tx.type === "withdrawal" ? "bg-rose-500/10" : tx.type === "purchase" ? "bg-blue-500/10" : tx.type === "sale" ? "bg-purple-500/10" : "bg-amber-500/10"}`}>
                        {tx.type === "deposit" ? <ArrowDownLeft className="h-4 w-4 text-emerald-400" /> :
                          tx.type === "withdrawal" ? <ArrowUpRight className="h-4 w-4 text-rose-400" /> :
                            tx.type === "purchase" ? <Coins className="h-4 w-4 text-blue-400" /> :
                              tx.type === "sale" ? <Send className="h-4 w-4 text-purple-400" /> :
        ...
        {activeTab === "market" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => { setSelectedShare(shares[0] || null); setP2pDialog(true); }}
            ...
            {activeListings.length === 0 ? (
              ...
              <div key={l.id} className="rounded-2xl border border-white/5 p-5" style={{ background: "rgba(255,255,255,0.02)" }}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-mono font-bold text-white/90">{l.companies?.name}</p>
          ...
          <div className="space-y-3">
            <div>
              ...
              >
                ...
                {paymentServices.map((s: any) => <option key={s.id} value={s.id}>{s.name} ({s.contact})</option>)}
              </select>
            ...
            <div>
              ...
              <input
                placeholder="ID/Référence du transfert"
                value={paymentRef}
      ...
      <Dialog open={p2pDialog} onOpenChange={setP2pDialog}>
        <DialogContent style={{ background: "#0a0f1e", border: "1px solid rgba(168,85,247,0.2)" }}>
          ...
          <div className="space-y-3">
            <div>
              ...
              <select
                ...
                onChange={e => {
                  const sh = shares.find(s => s.id === e.target.value);
                  setSelectedShare(sh || null);
                  if (sh) {
                    setP2pQty(String(sh.quantity));
                    setP2pPrice(String(Number(sh.companies?.price_per_share ?? sh.purchase_price)));
                  }
                }}
              ...
              >
                ...
                {shares.map(s => (
                  <option key={s.id} value={s.id}>
                    {s.companies?.name || "?"} · {s.quantity} titre(s)
                  </option>
                ))}
              </select>
            </div>
          ...
          <DialogFooter>
            ...
            <button
              onClick={() => p2pMutation.mutate()}
              disabled={p2pMutation.isPending || !selectedShare}
